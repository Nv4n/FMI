...
void ModifiableIntegerFunction::serialize(const char *fileName) {
    ...
    writer.write(reinterpret_cast<const char *>(&func), sizeof(INTEGER_FUNCTION));
    writer.close();
}
...
void ModifiableIntegerFunction::deserialize(const char *fileName) {
    ...
    reader.read(reinterpret_cast< char *>(&func), sizeof(INTEGER_FUNCTION));
    reader.close();
}
...
bool ModifiableIntegerFunction::operator>(const ModifiableIntegerFunction &other) const {
    for (size_t i = 0; i < length; ++i) {
        for (size_t j = 0; j < length; ++j) {
            if (changedVals[i].inputValue != other.changedVals[j].inputValue) {
                continue;
            }
            if (changedVals[i].isExcluded && !other.changedVals[j].isExcluded) {
                return false;
            }
            if (!changedVals[i].isExcluded && other.changedVals[j].isExcluded) {
                return true;
            }
            if (changedVals[i].outputValue < other.changedVals[j].outputValue) {
                return false;
            }
        }
    }
    return func.target<int16_t(*)(int16_t)>() < func.target<int16_t(*)(int16_t)>();
}
