...
bool TransactionHelper::addTransaction(unsigned int sender,
    ...
    if (fileExist && std::remove("blocks.dat") != 0) {
        std::remove("tempblocks.dat");
        std::cout << "Transaction failed" << std::endl;
        return false;
    } else if (std::rename("tempblocks.dat", "blocks.dat") != 0) {
        std::remove("tempblocks.dat");
        std::cout << "Transaction failed" << std::endl;
        return false;
    } else {
...
void TransactionHelper::verifyTransactions() {
    ...
    while (!in.eof()) {
        ...
        if ((currentId == 0 && tsblock.prevBlockId != 0)
            || currentId <= prevId
            || (currentId != 0 && prevHash != tsblock.prevBlockHash)) {
