...
-- TASK01
SELECT LASTNAME, SALARY, (SALARY * 1.05) AS INC_Y_SALARY, DECIMAL((SALARY / 12.0 * 1.05), 10, 2) AS INC_M_SALARY
FROM EMP
...
SELECT PROJNO,
       ...
       PRENDATE+ (ABS(DAYS(PRENDATE)-DAYS( PRSTDATE))*0.1) DAYS AS EXPECTED
FROM PROJ
WHERE PROJNO LIKE 'MA%'
ORDER BY PROJNO;
